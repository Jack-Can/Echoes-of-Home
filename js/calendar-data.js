...
function generateChatRecords() {
  ...
  function createMessage(id, time, sender, type, content, extra = {}) {
    return {
      ...
      time,
      sender,
      senderName: sender === 'parent' ? '妈妈' : '小天',
      type,
      content,
      warmthMarked: false,
      warmthTag: null,
...
CalendarApp.data.anniversaries = [
  ...
  { date: '10-08', title: '小天生日', type: 'birthday', icon: '🎂' },
...
CalendarApp.data.memoryHooks = {
  ...
  child: {
    name: '小天',
    traits: ['在外地上大学', '经常泡图书馆', '喜欢吃妈妈做的红烧肉'],
...
CalendarApp.data.aiSuggestions = {
  ...
  dailyTips: {
    ...
    cloudy: [
      '好久没跟爸妈聊天了，要不要发个消息报平安？',
      '虽然忙，但别忘了给爸妈发个问候 📝',
      '今天过得怎么样？跟爸妈分享一下最近的生活吧'
    ]
  }
};
